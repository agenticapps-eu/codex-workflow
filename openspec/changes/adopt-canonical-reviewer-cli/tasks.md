...
- [ ] 1.1 Vendor `tools/reviewer-cli-conformance.sh` byte-identical from core
      `60cd83f`. No edits, no header. This is the instrument.
- [ ] 1.2 `tdd="true"` — RED: add `test_migration_0015` asserting the harness
...
- [ ] 1.3 `tdd="true"` — RED: assert `bin/reviewer-cli.sh` carries a
      ...
      Observe it fail.
- [ ] 1.4 `tdd="true"` — RED: add the conditional byte-identity assertion for
...
- [ ] 1.5 `tdd="true"` — RED: assert `tools/core-vendor.manifest` **lists all
...
- [ ] 1.6 GREEN: vendor `bin/reviewer-cli.sh` byte-identical from core `60cd83f`.
      1.2, 1.3 and 1.4 go green. Do not hand-merge this repo's superset into it —
      the canonical already carries all four arms.
- [ ] 1.7 GREEN: update `tools/core-vendor.manifest` — advance `core_commit`
...
- [ ] 2.1 `tdd="true"` — RED: assert `bin/install-gate.sh --marker
...
- [ ] 2.2 `tdd="true"` — RED: assert the same invocation **upgrades** an
...
- [ ] 2.3 **Regression pin — not `tdd="true"`, and deliberately so.** Assert that
...
- [ ] 2.4 GREEN: parameterise `bin/install-gate.sh` — `--marker <name>`,
...
- [ ] 2.5 `tdd="true"` — RED: assert `install.sh` writes the shared wrapper
      **through** `install-gate.sh` and that no unarbitrated
      `install -m 0755 …reviewer-cli.sh` remains anywhere in the repo. Observe it
      fail.
- [ ] 2.6 GREEN: rewire the `install.sh` call site with the same three-way `rc`
...
- [ ] 3.2 GREEN: update the skill's resolution snippet to match the installer's
...
- [ ] 4.1 `tdd="true"` — RED: assert `.github/workflows/openspec-gate.yml` runs
      `tools/reviewer-cli-conformance.sh bin/reviewer-cli.sh` **before** the step
      that runs the gate. Observe it fail.
- [ ] 4.2 GREEN: add the step, with a comment naming why the order matters — a
...
- [ ] 5.1 Write `migrations/0015-adopt-canonical-reviewer-cli.md` (1.1.0 → 1.2.0)
      ...
      is not.
- [ ] 5.2 Write `docs/decisions/0013-vendor-canonical-reviewer-cli.md` — the
...
- [ ] 5.3 Update `CHANGELOG.md` under `## [1.2.0]`, with the before/after harness
      numbers and the `--family` fleet table.
- [ ] 5.4 **Mutation-check every new row.** For each assertion added above,
...
- [ ] 5.5 Full evidence pass: `migrations/run-tests.sh` (record PASS/FAIL/SKIP
...
- [ ] 6.1 `/cso` over the diff → `SECURITY.md`. The change touches an installer
      that writes a path shared by every host on the machine, which is the reason
      this is not conditional-away.
- [ ] 6.2 Stage-2 independent code review in a fresh context →
      `CODE-REVIEW.md`. `openspec validate` is a spec check and does not
      discharge it.
- [ ] 6.3 Archive the change (fold the `reviewer-cli` delta into
