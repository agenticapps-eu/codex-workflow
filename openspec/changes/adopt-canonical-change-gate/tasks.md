...
- [ ] 1.1 Vendor `tools/change-gate-conformance.sh` byte-identical from core
      `ae90483` into a new `tools/` dir, with a vendor header naming the commit.
      No behaviour change — this is the instrument.
- [ ] 1.2 `tdd="true"` — RED: add `test_migration_0014` asserting the harness
...
- [ ] 1.3 `tdd="true"` — RED: pin GAP-1 (code edit from a subdirectory blocks,
...
- [ ] 1.4 GREEN: vendor `bin/openspec-change-gate.sh` byte-identical from core
      `ae90483`, with the same vendor header. 1.2 and 1.3 go green.
- [ ] 1.5 `tdd="true"` — add the conditional byte-identity assertion: compare both
...
- [ ] 1.6 `tdd="true"` — assert the two vendor headers name the **same** core
...
- [ ] 2.1 `tdd="true"` — RED: assert `bin/git-hooks/pre-commit` blocks a staged
      ...
      `tool_input` payload.
- [ ] 2.2 `tdd="true"` — RED: assert the absent-gate path fails **open** — exit 0
...
- [ ] 2.3 `tdd="true"` — RED: assert the shared copy is ignored in favour of
      repo-local when its `# gate-version:` marker is absent or malformed, and
      that the fallback warns.
- [ ] 2.4 GREEN: rewrite `bin/git-hooks/pre-commit` as a thin wrapper satisfying
...
- [ ] 2.5 `tdd="true"` — RED: assert `bin/openspec-gate-ci.sh` runs the harness
      before the gate, invokes `--ci`, resolves the **repo-local** gate, and no
      longer takes a base-ref argument.
- [ ] 2.6 GREEN: rewrite `bin/openspec-gate-ci.sh` accordingly, preserving the
      filename and its no-op-without-a-spec-slot behaviour.
- [ ] 2.7 Update `.github/workflows/openspec-gate.yml`: add the conformance step
      before the gate step, drop the `BASE_SHA` plumbing `--ci` no longer needs,
      and set `OPENSPEC_GATE_SELF: codex` in the job env.
- [ ] 2.8 `tdd="true"` — RED then GREEN: assert the `PreToolUse` wrapper falls
...
- [ ] 2.9 Verify migration 0013 does not break: `run-tests.sh` asserts only that
...
- [ ] 3.1 `tdd="true"` — RED: assert `install.sh` leaves an installed gate
...
- [ ] 3.2 `tdd="true"` — RED: assert the equal-version case installs, and that an
      absent or malformed marker (`1.2`, `1.3.0-rc1`, empty) is treated as
      `0.0.0` and installs.
- [ ] 3.3 GREEN: implement the marker read, the portable three-field numeric
      compare (`sort -t. -k1,1n -k2,2n -k3,3n`, no GNU-only version-sort), and
      the refusal message.
- [ ] 3.4 `tdd="true"` — RED then GREEN: make the write atomic (temp file in the
...
- [ ] 3.5 `tdd="true"` — assert the fresh-machine bootstrap: no
      `~/.agenticapps/bin/` directory at all, and the install still succeeds.
- [ ] 3.6 Verify live against a real `$AA_BIN` copy, and confirm `--dry-run`
      reports the decision without writing.

## 4. Self-review exclusion

- [ ] 4.1 Cross-surface regression guard (**not** `tdd`, deliberately): assert all
...
- [ ] 4.2 `tdd="true"` — RED: assert two `## Reviewer: gemini` headings count as
      one independent reviewer and block.
- [ ] 4.3 GREEN: export it in
...
- [ ] 4.4 Re-point the wrapper's GAP-1 assertion in `migrations/run-tests.sh`: the
...
- [ ] 5.1 Delete `docs/GATE-KNOWN-GAPS.md` and remove `test_gate_known_gaps` and
      its entry in the test runner's list.
- [ ] 5.2 Write `docs/decisions/0012-vendor-canonical-change-gate.md`, carrying
...
- [ ] 5.3 Remove every remaining `GATE-KNOWN-GAPS.md` reference — including its
...
- [ ] 6.1 Write `migrations/0014-adopt-canonical-gate.md`, 1.0.0 → 1.1.0, with a
      real Apply block. `run-tests.sh` extracts and executes it — do not
      re-implement the steps inline.
- [ ] 6.2 Bump `skills/agentic-apps-workflow/SKILL.md` `version:` and
      `.codex/workflow-version.txt` to 1.1.0 so `test_drift` stays green.
- [ ] 6.3 Update `CHANGELOG.md`, `docs/WORKFLOW.md`, and the §18 line in the
...
- [ ] 7.1 Full suite green: `bash migrations/run-tests.sh`. Baseline is
      520 PASS / 0 FAIL / 1 SKIP; report the new counts.
- [ ] 7.2 Report the harness result — every declared row passing, zero failures —
      in the PR, with the row count as observed rather than as a constant.
- [ ] 7.3 Live floor proof: attempt a real `git commit` of a code file under an
...
- [ ] 7.5 `/cso` — the gate is the enforcement trust boundary and this change
