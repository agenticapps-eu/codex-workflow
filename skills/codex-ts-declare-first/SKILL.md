---
name: codex-ts-declare-first
version: 0.1.0
implements_spec: 0.4.0
implements_gate: tdd
description: |
  Declare-first TypeScript discipline for new modules in TS-primary
  AgenticApps projects on the Codex host, per workflow-core spec §13.
...
# codex-ts-declare-first — declare-first TypeScript discipline (spec §13)

This skill implements the declare-first TypeScript discipline from
`agenticapps-workflow-core` spec §13 on the OpenAI Codex CLI host. When
...
It is the Codex-host equivalent of claude-workflow's `ts-declare-first`
skill. §13 names `codex-workflow` explicitly as a TS-targeting host that
...
- **Explicit** (this skill's current trigger): operator invokes
  `$codex-ts-declare-first` when starting a new TS module. E.g.:
...
`codex-verification` evidence shape `test_output`):
...
flowchart TD
  ...
  proceed --> done[Three atomic commits land — §06 evidence satisfied → REPORT to codex-verification]
```
...
that `codex-verification` checks before a task completes:

| Phase | Evidence (`codex-verification` shape) |
|---|---|
...
`codex-verification` MUST refuse task completion if the three-commit
